...
output_path_bilinear = r"D:\Agri_Geospatial\Resampled\Prisma_downscaled_area2_BILINEAR.tif"
os.makedirs(os.path.dirname(output_path_bilinear), exist_ok=True)
...
# Perform the resampling using Bilinear
resampled_ds = gdal.Warp(
    destNameOrDestDS=output_path_bilinear,
    ...
    resampleAlg='bilinear',   # Bilinear interpolation
    multithread=True,
...
if resampled_ds is None:
    raise RuntimeError("GDAL Warp failed (Bilinear).")
resampled_ds.FlushCache()
resampled_ds = None

print(f"Bilinear resampled output saved at: {output_path_bilinear}")
